...
export const TeacherListItem: React.FC<TeacherListItemProps> = ({ user, onDelete }) => (
  <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', padding: '0.9rem 1rem', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem', flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        {user.role !== 'admin' && (
          <button onClick={() => onDelete(user.id, user.fullName)} style={{ background: 'rgba(255,69,58,0.15)', border: '1px solid rgba(255,69,58,0.3)', color: '#ff453a', borderRadius: '6px', padding: '0.2rem 0.4rem', cursor: 'pointer' }} title="Hapus Akun Guru">
            <Trash2 size={13} />
          </button>
        )}
      </div>
